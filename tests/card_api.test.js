...
const initialDocs = require('./data/documents.json')
...
beforeEach(async () => {
  ...
  await Document.insertMany(initialDocs)
})
...
describe('DELETE card', () => {
  ...
  test.only('parent document is updated', async () => {
    const cardsAtStart = await helper.cardsInDb()
    const cardToDelete = cardsAtStart[0]
    let id = String(cardToDelete.document)
    console.log(cardToDelete);
    console.log(id);
    let parentDoc = await Document.findById(id).exec();
    ...
    expect(parentDoc.editorCardsAndGroups).not.toContain({ item: cardToDelete.id, cardOrGroup: "Card"})
  })
